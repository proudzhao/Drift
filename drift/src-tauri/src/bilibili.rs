...
pub async fn test_bilibili_api(room_id: u64) -> Result<Vec<ApiTestStep>, String> {
    ...
    let Some(room_init) = run_api_test_step(&mut steps, "room_init", "真实房间号", true, async {
        let data = fetch_room_init(room_id).await?;
        let detail = format!(
    ...
    let Some(mixin_key) = run_api_test_step(&mut steps, "wbi_nav", "WBI key", true, async {
        let data = fetch_wbi_mixin_key(&device.cookie).await?;
    ...
    let Some(wbi_params) = run_api_test_step(&mut steps, "wbi_sign", "WBI 签名", true, async {
        let params = build_wbi_params_with_mixin_key(room_init.room_id, &mixin_key)?;
    ...
    let Some(danmu_info) = run_api_test_step(
        ...
        true,
        async {
            let data = fetch_danmu_info_with_params(room_init.room_id, &device.cookie, &wbi_params)
                .await?;
            let host = data
                .host_list
                .first()
...
async fn run_anchor_name_test_step(
    ...
    let started_at = Instant::now();
    let result = if let Some(anchor_name) = cached_anchor_name(uid) {
        Ok((
            "success",
            "主播名缓存命中".to_string(),
        ...
        match fetch_room_base_anchor_name(room_id, uid, cookie).await {
            ...
            Err(room_base_error) => match fetch_anchor_name(uid, cookie).await {
                Ok(anchor_name) => {
                    cache_anchor_name(uid, &anchor_name);
                    Ok((
                        "warning",
                ...
                Err(profile_error) => match fetch_room_info(room_id, cookie).await {
                    Ok(room_info) => {
...
async fn fetch_danmu_info_with_params(
    room_id: u64,
    cookie: &str,
...
fn build_wbi_params_with_mixin_key(
    room_id: u64,
    mixin_key: &str,
